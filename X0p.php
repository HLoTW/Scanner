...
elseif ($ip == "fix")
  {
    ...
    echo $bold . $fgreen . "[i] Job finished successfully! Please Restart X0p \n";
    exit;
  }
        ...
        elseif ($scan == "8")
          {
            ...
            if (count($array) > 0)
              {
                userinput("Do You Want X0p To Detect CMS Of The Sites? [Y/N]");
            ...
            if (strpos($srccd, "wp-content") !== false)
              {
                ...
                if ($wp_version != "")
                  ...
                  {
                    $reallink = $ipsl . $ip;
                    echo $red . "Failed \n\n[!] X0p could not determine the WordPress version of the target!";
                    echo "\n\n";
                    echo $bold . $yellow . "[*] Scanning Complete. Press Enter To Continue OR CTRL + C To Stop\n\n";
                    trim(fgets(STDIN, 1024));
                    goto scanlist;
        ...
        elseif ($scan == 'U' || $scan == 'u')
          {
            ...
            if ($latestversion > $rhversion)
              {
                echo $bold . $fgreen . "\n\n[U] Update Available, Please Update Your Version Of X0p \n" . $cln;
                echo $bold . $white . "    Link: https://github.com/Tuhinshubhra/RED_HAWK\n\n" . $cln;
              }

            elseif ($rhversion == $latestversion)
              {
                echo $bold . $fgreen . "\n[i] You are already running the latest version of X0p. \n\n" . $cln;
              }
            else
              {
